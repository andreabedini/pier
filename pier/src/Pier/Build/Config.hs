...
data Config = Config
    ...
    , localPackages :: Map PackageName (Artifact, Version)
...
askConfig = do
    ...
    return Config
        { plan = p
        , configGhc = ghc
        , localPackages = Map.fromList pkgDescs
        , configExtraDeps = Map.fromList [ (packageName pkg, packageVersion pkg)
                                        | pkg <- extraDeps yaml
                                        ]
...
resolvePackage conf n
    ...
    | Just v <- Map.lookup n (corePackageVersions $ plan conf)
                = Builtin $ PackageIdentifier n v
    | Just (a, v) <- Map.lookup n (localPackages conf)
                = Local a $ PackageIdentifier n v
    -- Extra-deps override packages in the build plan:
    | Just v <- Map.lookup n (configExtraDeps conf)
                = Hackage (PackageIdentifier n v) Map.empty
    | Just p <- Map.lookup n (planPackages $ plan conf)
